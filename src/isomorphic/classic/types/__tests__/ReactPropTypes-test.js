...
describe('ReactPropTypes', () => {
  ...
  describe('ArrayOf Type', () => {
    ...
    it('should support arrayOf with complex types', () => {
      typeCheckPass(PropTypes.arrayOf(PropTypes.shape({ a: PropTypes.number.isRequired })), [
        { a: 1 },
        { a: 2 },
      ]);

      function Thing() {}
      typeCheckPass(PropTypes.arrayOf(PropTypes.instanceOf(Thing)), [new Thing(), new Thing()]);
    });
    ...
    it('should warn if called manually in development', () => {
      spyOn(console, 'error');
      expectWarningInDevelopment(PropTypes.arrayOf({ foo: PropTypes.string }), { foo: 'bar' });
      expectWarningInDevelopment(PropTypes.arrayOf(PropTypes.number), [1, 2, 'b']);
  ...
  describe('ObjectOf Type', () => {
    ...
    it('should support objectOf with complex types', () => {
      typeCheckPass(PropTypes.objectOf(PropTypes.shape({ a: PropTypes.number.isRequired })), {
        a: { a: 1 },
        b: { a: 2 },
      });
    ...
    it('should warn for missing required values', () => {
      typeCheckFailRequiredValues(PropTypes.objectOf(PropTypes.number).isRequired);
    });

    it('should warn if called manually in development', () => {
      spyOn(console, 'error');
      expectWarningInDevelopment(PropTypes.objectOf({ foo: PropTypes.string }), { foo: 'bar' });
      expectWarningInDevelopment(PropTypes.objectOf(PropTypes.number), { a: 1, b: 2, c: 'b' });
  ...
  describe('Union Types', () => {
    ...
    it('should warn if none of the types are valid', () => {
      ...
      typeCheckFail(checker, { c: 1 }, 'Invalid prop `testProp` supplied to `testComponent`.');
    });
    ...
    it('should be implicitly optional and not warn without values', () => {
      typeCheckPass(PropTypes.oneOfType([PropTypes.string, PropTypes.number]), null);
      typeCheckPass(PropTypes.oneOfType([PropTypes.string, PropTypes.number]), undefined);
    });
    ...
    it('should warn if called manually in development', () => {
      spyOn(console, 'error');
      expectWarningInDevelopment(PropTypes.oneOfType([PropTypes.string, PropTypes.number]), []);
      expectWarningInDevelopment(PropTypes.oneOfType([PropTypes.string, PropTypes.number]), null);
  ...
  describe('Shape Types', () => {
    ...
    it('should be implicitly optional and not warn without values', () => {
      typeCheckPass(PropTypes.shape(PropTypes.shape({ key: PropTypes.number })), null);
      typeCheckPass(PropTypes.shape(PropTypes.shape({ key: PropTypes.number })), undefined);
    });

    it('should warn for missing required values', () => {
      typeCheckFailRequiredValues(PropTypes.shape({ key: PropTypes.number }).isRequired);
    });
    ...
    it('should warn if called manually in development', () => {
      ...
      expectWarningInDevelopment(PropTypes.shape({ key: PropTypes.number }).isRequired, null);
      expectWarningInDevelopment(PropTypes.shape({ key: PropTypes.number }).isRequired, undefined);
      expectWarningInDevelopment(PropTypes.element, <div />);
    });
  });
  ...
  describe('Custom validator', () => {
    ...
    it("should have received the validator's return value", () => {
      spyOn(console, 'error');
      var spy = jasmine.createSpy().and.callFake(function(props, propName, componentName) {
        if (props[propName] !== 5) {
