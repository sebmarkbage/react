...
describe('ReactNativeAttributePayload', () => {
  it('should work with simple example', () => {
    expect(diff({ a: 1, c: 3 }, { b: 2, c: 3 }, { a: true, b: true })).toEqual({ a: null, b: 2 });
  });
  ...
  it('should use the diff attribute', () => {
    ...
    expect(
      diff({ a: [1], b: [3] }, { a: [2], b: [4] }, { a: { diff: diffA }, b: { diff: diffB } })
    ).toEqual({ a: [2] });
  ...
  it('should not use the diff attribute on addition/removal', () => {
    var diffA = jest.fn();
    var diffB = jest.fn();
    expect(diff({ a: [1] }, { b: [2] }, { a: { diff: diffA }, b: { diff: diffB } })).toEqual({
      a: null,
      b: [2],
    });
  ...
  it('should work with undefined styles', () => {
    expect(
      diff({ style: { a: '#ffffff', b: 1 } }, { style: undefined }, { style: { b: true } })
    ).toEqual({ b: null });
    expect(
      diff({ style: undefined }, { style: { a: '#ffffff', b: 1 } }, { style: { b: true } })
    ).toEqual({ b: 1 });
    expect(diff({ style: undefined }, { style: undefined }, { style: { b: true } })).toEqual(null);
  });
  ...
  it('should flatten nested styles and predefined styles', () => {
    ...
    expect(diff({}, { someStyle: [{ foo: 1 }, { bar: 2 }] }, validStyleAttribute)).toEqual({
      foo: 1,
      bar: 2,
    });
    ...
    expect(
      diff({}, { someStyle: [[{ foo: 1 }, { foo: 2 }], barStyle] }, validStyleAttribute)
    ).toEqual({ foo: 2, bar: 3 });
  });
