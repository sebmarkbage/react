...
var _touchConfig = function(
  ...
  return {
    nativeEvent: touchEvent(targetNodeHandle, activeTouchObjects, changedTouchObjects),
...
var startConfig = function(nodeHandle, allTouchHandles, changedIndices) {
  return _touchConfig('topTouchStart', nodeHandle, allTouchHandles, changedIndices, nodeHandle);
};
...
var moveConfig = function(nodeHandle, allTouchHandles, changedIndices) {
  return _touchConfig('topTouchMove', nodeHandle, allTouchHandles, changedIndices, nodeHandle);
};
...
var endConfig = function(nodeHandle, allTouchHandles, changedIndices) {
  return _touchConfig('topTouchEnd', nodeHandle, allTouchHandles, changedIndices, nodeHandle);
};
  ...
  for (var eventName in ret) {
    for (var readableNodeName in readableIDToID) {
      if (ret[eventName].bubbled) {
        ...
        ret[eventName][readableNodeName] = { order: NA, assertEvent: null, returnVal: undefined };
      }
    }
  }
  return ret;
...
describe('ResponderEventPlugin', () => {
  ...
  it('should grant responder grandParent while capturing', () => {
    ...
    run(config, three, startConfig(three.child, [three.child], [0]));
    expect(ResponderEventPlugin._getResponder()).toBe(getInstanceFromNode(three.grandParent));
  ...
  it('should grant responder parent while capturing', () => {
    ...
    run(config, three, startConfig(three.child, [three.child], [0]));
    expect(ResponderEventPlugin._getResponder()).toBe(getInstanceFromNode(three.parent));
  ...
  it('should grant responder child while capturing', () => {
    ...
    run(config, three, startConfig(three.child, [three.child], [0]));
    expect(ResponderEventPlugin._getResponder()).toBe(getInstanceFromNode(three.child));
  ...
  it('should negotiate until first common ancestor when there are siblings', () => {
    ...
    expect(ResponderEventPlugin._getResponder()).toBe(getInstanceFromNode(siblings.childOne));
    ...
    var touchConfig = startConfig(siblings.childTwo, [siblings.childOne, siblings.childTwo], [1]);
  ...
  it('should cancel correctly', () => {
    ...
    var nativeEvent = _touchConfig('topTouchCancel', three.child, [three.child], [0]);
