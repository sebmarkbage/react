...
describe('ReactIncrementalSideEffects', () => {
  ...
  it('can update child nodes of a fragment', function() {
      ...
      return (
        <div>
          ...
          {props.text === 'World'
            ...
            : props.text === 'Hi' ? [<div key="b" />, <Bar key="a" text={props.text} />] : null}
          <span prop="test" />
        </div>
      );
    }
    ...
    ReactNoop.render(<Foo text="World" />);
    ReactNoop.flush();
    expect(ReactNoop.getChildren()).toEqual([div(span(), span(), div(), span('test'))]);

    ReactNoop.render(<Foo text="Hi" />);
    ReactNoop.flush();
    expect(ReactNoop.getChildren()).toEqual([div(span(), div(), span(), span('test'))]);
  });
  ...
  it('can deletes children either components, host or text', function() {
    ...
    ReactNoop.flush();
    expect(ReactNoop.getChildren()).toEqual([div(div(), span('Hello'), 'World')]);
    ...
    function Foo(props) {
      return (
        <div>
          {props.useClass
            ? <ClassComponent />
            : props.useFunction ? <FunctionalComponent /> : props.useText ? 'Text' : null}
          Trail
        </div>
      );
    }
  ...
  it('does not update child nodes if a flush is aborted', () => {
    ...
    ReactNoop.render(<Foo text="Hello" />);
    ReactNoop.flush();
    expect(ReactNoop.getChildren()).toEqual([div(div(span('Hello'), span('Hello')), span('Yo'))]);
  ...
  it('can reuse side-effects after being preempted', () => {
    ...
    expect(ReactNoop.getChildren()).toEqual([div(div(span('Hi'), span('foo')))]);
    ...
    expect(ReactNoop.getChildren()).toEqual([div(div(span('Hello'), span('World')))]);
  });
  ...
  it('can defer side-effects and resume them later on', function() {
    ...
    expect(ReactNoop.getChildren()).toEqual([
      div(span(0), div /*the spans are down-prioritized and not rendered yet*/()),
    ]);
    ReactNoop.render(<Foo tick={1} idx={0} />);
    ReactNoop.flushDeferredPri(35 + 25);
    expect(ReactNoop.getChildren()).toEqual([div(span(1), div /*still not rendered yet*/())]);
  ...
  it('deprioritizes setStates that happens within a deprioritized tree', () => {
    ...
    ReactNoop.render(<Foo tick={0} idx={0} />);
    ReactNoop.flush();
    expect(ReactNoop.getChildren()).toEqual([div(span(0), div(span(0), span(0), span(0)))]);
  ...
  it('calls componentWillUnmount after a deletion, even if nested', () => {
    ...
    ReactNoop.render(<Foo show={false} />);
    ReactNoop.flush();
    expect(ops).toEqual(['A', 'Wrapper', 'B', 'C', 'Wrapper', 'D', 'E', 'F', 'G']);
  });
