...
function ChildReconciler(shouldClone, shouldTrackSideEffects) {
  ...
  function createChild(returnFiber: Fiber, newChild: any, priority: PriorityLevel): Fiber | null {
  ...
  function warnOnDuplicateKey(child: mixed, knownKeys: Set<string> | null): Set<string> | null {
    if (__DEV__) {
  ...
  function reconcileChildrenArray(
    ...
    if (oldFiber === null) {
      ...
      for (; newIdx < newChildren.length; newIdx++) {
        const newFiber = createChild(returnFiber, newChildren[newIdx], priority);
        if (!newFiber) {
          continue;
        }
        lastPlacedIndex = placeChild(newFiber, lastPlacedIndex, newIdx);
  ...
  function reconcileChildrenIterator(
    ...
    // Keep scanning and use the map to restore deleted items as moves.
    for (; !step.done; newIdx++, step = newChildren.next()) {
      const newFiber = updateFromMap(existingChildren, returnFiber, newIdx, step.value, priority);
      if (newFiber !== null) {
        if (shouldTrackSideEffects) {
          if (newFiber.alternate !== null) {
            // The new fiber is a work in progress, but if there exists a
  ...
  function reconcileChildFibers(
        ...
        switch (newChild.$$typeof) {
          case REACT_ELEMENT_TYPE:
            return placeSingleChild(
              reconcileSingleElement(returnFiber, currentFirstChild, newChild, priority)
            );

          case REACT_PORTAL_TYPE:
            return placeSingleChild(
              reconcileSinglePortal(returnFiber, currentFirstChild, newChild, priority)
            );
        }
      } else {
        switch (newChild.$$typeof) {
          ...
          case REACT_COROUTINE_TYPE:
            return placeSingleChild(
              reconcileSingleCoroutine(returnFiber, currentFirstChild, newChild, priority)
            );

          case REACT_YIELD_TYPE:
            return placeSingleChild(
              reconcileSingleYield(returnFiber, currentFirstChild, newChild, priority)
            );
    ...
    if (typeof newChild === 'string' || typeof newChild === 'number') {
      return placeSingleChild(
        reconcileSingleTextNode(returnFiber, currentFirstChild, '' + newChild, priority)
      );
    }
