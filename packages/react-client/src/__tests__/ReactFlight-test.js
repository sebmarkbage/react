...
describe('ReactFlight', () => {
  ...
  it('shares state between single return and array return in a parent', async () => {
    ...
    console.log(transport.map(c => Buffer.from(c).toString('utf8')).join('\n'))
    ...
    console.log(transport2.map(c => Buffer.from(c).toString('utf8')).join('\n'))
  ...
  it('preserves state with keys split across async work', async () => {
    ...
    const transport = ReactNoopFlightServer.render(<Item key="a" name="A" />);
    ...
    const transport2 = ReactNoopFlightServer.render(<Item key="a" name="B" />);
